...
## 0. Seed demo data (before you start)

Before presenting, fill the database with realistic demo data in one command, so every screen below has meaningful content:

```bash
docker compose exec api python -m app.scripts.seed_demo_data
```

This is safe to run more than once (it won't create duplicates). See the "Demo Seed Data" section in [README.md](../README.md) for the account list.
...
9. **Driver location map (Kurye Haritası)** — show the live map with driver positions and their current active route. With seeded demo data, `driver1` and `driver2` appear with a position and an active route, while `driver3` appears with no saved location — a good moment to point out that the map handles missing location data gracefully.
...
15. **Driver login and driver flow** — log out, log in as `driver1@logipulse.demo` (or `driver2`), and show that the sidebar only has "Rotalarım" and "Konumumu Güncelle" (no admin pages). Open an assigned route, deliver/fail/skip a stop, and update the driver's location.
